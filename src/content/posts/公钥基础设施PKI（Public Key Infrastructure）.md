...
## 数字证书（Digital Certificate）：信任的最小单元

前面把数字证书比作「数字身份证」，这一节拆开看它到底长什么样。PKI 的一切动作——签发、验证、吊销——最终都落在这张「身份证」上。

### 数字证书的本质

**数字证书**是一份由 CA 用私钥签名、把「**身份信息 + 公钥**」绑定在一起的电子文档。它回答的核心问题只有一个：**「这份公钥属于谁」**。你信任 CA，CA 担保公钥与身份的对应关系，于是两个从未直接接触的实体也能互相信任。

一句话公式：

**数字证书 = 身份信息 + 公钥 + CA 的数字签名**

- **身份信息**：证书持有者是谁（域名 / 组织 / 个人）。
- **公钥**：持有者的公钥，用于加密数据或验证签名。
- **CA 签名**：CA 用私钥对前两部分内容做的签名，是证书的「防伪封条」。

关键认知：**证书里只有公钥，私钥绝不出现在证书中**。私钥由持证方自己秘密保管，证书本身可以公开分发、随意复制——这正是下文自测题第 4 题要考的点。

### X.509 v3 证书的标准字段

互联网通用证书标准是 **X.509**（目前为 v3），标准字段如下：

| 字段                  | 含义                                                         |
| --------------------- | ------------------------------------------------------------ |
| 版本号 Version        | v1 / v2 / v3；只有 v3 支持扩展字段，现代证书均为 v3          |
| 序列号 Serial Number  | CA 分配的唯一编号；吊销时凭它在 CRL / OCSP 中定位这张证书    |
| 签名算法 Signature Alg. | CA 使用的签名算法，如 SHA256withRSA                        |
| 颁发者 Issuer         | 签发该证书的 CA 的 DN（可识别名）                            |
| 有效期 Validity       | Not Before ~ Not After；过期即自然失效，与吊销是两回事       |
| 主体 Subject          | 证书持有者的 DN；服务器证书通常 CN=域名                      |
| 主体公钥 Subject Public Key Info | 持有者的公钥及算法（如 RSA 2048、ECDSA P-256）    |
| 扩展 Extensions       | v3 新增，约束证书的用途，见下                                |
| 签名值 Signature      | CA 对证书全部内容的签名，防篡改核心                          |

### 扩展字段：证书用途的「紧箍咒」

v3 扩展让证书不再只是「公钥 + 名字」，而是能精确限定它能干什么、不能干什么：

- **Basic Constraints**：标记是否为 CA 证书（`CA:TRUE`）、路径长度约束 Path Length。
- **Key Usage**：基础用途位，如 `digitalSignature`（数字签名）、`keyEncipherment`（密钥加密）、**`keyCertSign`（为下级证书签名，CA 证书必须含）**。
- **Extended Key Usage（EKU）**：细分用途，如 **`serverAuth`**（TLS 服务端）、`clientAuth`（TLS 客户端）、`codeSigning`（代码签名）。
- **Subject Alternative Name（SAN）**：证书绑定的域名 / IP 列表。现代浏览器只认 SAN、不再看 Subject 里的 CN，因此一张证书可同时覆盖多个域名。
- **CRL Distribution Points（CDP）**：CRL 名单的下载地址。
- **Authority Information Access（AIA）**：OCSP 查询地址等。
- **SCT（Signed Certificate Timestamp）**：CT 日志签发的时间戳，证明证书已登记进 CT 日志。

「信任链与路径验证」一节提到的「核对密钥用途」就发生在这几个字段上：CA 证书缺 `keyCertSign`、TLS 服务端证书缺 `serverAuth`，都会被判定用途不符而拒绝。

### 证书如何防篡改

1. **签发**：CA 对证书所有字段做哈希，用自己的私钥对哈希值签名，把签名值附在证书末尾。
2. **验证**：验证方用 CA 的公钥对签名进行验签（对证书内容重新计算哈希并与签名中携带的哈希比对）。一致 → 证书未被改动；不一致 → 证书被篡改。

由于只有 CA 拥有私钥，任何人都无法伪造签名；任何对证书内容的改动都会导致哈希对不上。这就是「CA 签名 = 防伪封条」的机制。

想亲手解析一张真实证书、逐字段对照上述内容？用下面这条命令把证书全部字段打印出来（详细说明见同系列《TLS证书相关内容学习(一)》）：

```bash
echo | openssl s_client -connect 域名:443 2>/dev/null | openssl x509 -noout -text
```
...
**【题 5】某 TLS 服务器证书在验证时因「用途不符」被浏览器拒绝。下列最可能的原因是？**

- A. 证书的 Basic Constraints 标记了 `CA:TRUE`
- B. 证书的 Extended Key Usage（EKU）缺少 `serverAuth`
- C. 证书的 Subject 未包含服务器域名
- D. 证书的序列号与证书库记录不一致

**答案：** B

**解析：** EKU 细分证书用途，TLS 服务端证书必须包含 `serverAuth`（Extended Key Usage）扩展，缺少即被判定用途不符（如 Chrome 提示「此网站的身份存疑/证书并非用于服务器认证」）。A 中 CA 证书缺 `keyCertSign`（Key Usage）同样会报用途错误，但 `CA:TRUE` 本身是 CA 证书的合法标记，方向相反；C 对应「域名不匹配」，属于身份校验而非用途校验；D 属于吊销状态校验。本题考 X.509 v3 扩展字段（Key Usage / EKU）在路径验证中的作用，是题 3、题 4 基础上的进阶考点。

**【题 6】某系统用员工的私钥对交易记录做数字签名。这一机制主要保障的安全目标是？**
